...
public class TempActivity extends AppCompatActivity {
    ...
    public static class DatePickerFragment extends DialogFragment
        ...
        public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
            ((TempActivity) getActivity()).alegeData(year, month, dayOfMonth, b1, b2);
        }
    }
    ...
    public static class TimePickerFragment extends DialogFragment
        ...
        public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
            ((TempActivity) getActivity()).alegeTimpul(hourOfDay, minute, b1, b2);
        }
        ...
        public void onClick(DialogInterface dialog, int which)
        {
            ...
            {
                ((TempActivity) getActivity()).stergeData(b1, b2);
                dialog.cancel();
            }
        }
        ...
        public void onClick(DialogInterface dialog, int which) {
            switch(which)
            {
                case DialogInterface.BUTTON_POSITIVE:
                {
                    ...
                    ((TempActivity) getActivity()).alegeDurata(ore, minute);
                    break;
                }
                case DialogInterface.BUTTON_NEGATIVE:
                {
                    ((TempActivity) getActivity()).stergeDurata();
                    dialog.cancel();
                    break;
                }
            }
        ...
        if(src.isChecked())
        {
            ...
            else
            {
                String text = "Until " + Integer.toString(deadline[2]) + "." + Integer.toString(deadline[1])
                        + "." + Integer.toString(deadline[2]) + " " + Integer.toString(deadline[3])
